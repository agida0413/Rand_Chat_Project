...
public class JoinDTO {
    ...
    @Pattern(regexp = "^[^\s][^\s]*$", message = "JoinDTO.닉네임은 공백으로 시작하거나 공백을 포함할 수 없음")
    @Pattern(regexp = "^[가-힣]+$", message = "JoinDTO.닉네임은 한글만 입력할 수 있습니다.")
    @Size(min = 3, max = 15)
    ...
    @Pattern(regexp = "^[가-힣]{2,8}$", message = "JoinDTO.이름은 2~8자의 한글만 입력할 수 있습니다.")
    private String name;
}
