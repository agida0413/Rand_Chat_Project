...
- *Filter는 Servlet 앞단에 위치하여 ControllerAdvice에 접근 불가*
  ...
  - <u>***ResponseApi 클래스에 필요한 예외처리 항목에 대해 response를 직접 보내는 메서드 작성***</u>
---

### <u>**검색어 추천 기능 중 한글 초,중,종성으로 검색 불가 문제**</u>

- *한글은 유니코드 체계상 초, 중, 종성 분리 검색 필요*

  - <u>***데이터베이스 내 초성, 중성, 종성 컬럼을 추가하여 별도로 저장 및 검색 컬럼 구성***</u>
---

### <u>**동일 유저가 중복 요청 시 데이터 일관성 깨짐 문제**</u>

- *같은 사용자에 대해 동일한 API 요청 시 충돌 방지 필요*

  - <u>***인터셉터와 ConcurrentHashMap을 이용하여 동일 사용자에 대한 중복 API 요청을 차단하는 로직 추가***</u>
---

### <u>**게시물 C, U 시 데이터베이스 트랜잭션 예외 발생으로 S3 파일 객체가 삭제되지 않는 문제**</u>

- *게시물 C, U 작업 실패 시 S3 파일 객체도 롤백되어야 함*

  - <u>***S3 업로드 로직을 먼저 실행하고 데이터베이스 트랜잭션 수행 후 예외 발생 시 생성된 URL과 매칭되는 S3 객체 삭제***</u>
---
